...
%
% gin_list_plabels.m		B Landeau 15/09/03
% ______________________________________________________________________
% 
%
...
text(0.20,y - dy/2,'x,y,z {mm}','FontSize',FS(9));
text(0.40,y - dy/2,'label','FontSize',FS(9))
text(0.60,y - dy/2,'%','FontSize',FS(9));
...
% defaults = L/R -> MNI = L/L
	...
	MNID=spm_vol(MNI);
	eval(['load ',strrep(MNI,'.img','_List.mat')]);
	%ROI.ID
	%ROI.Nom_C
	%ROI.Nom_L
	eval(['load ',strrep(MNI,'.img','_Border.mat')]);
	%BORDER_XYZ
	%BORDER_V
...
while prod(size(find(finite(Z))))
    ...
    %---------------------------------------------------------------
		...
		h     = text(0.18,y,sprintf(TabDat.fmt{1},XYZmm(:,i)),...
			'FontWeight','Bold',...
			'Tag','ListXYZ',...
			'ButtonDownFcn',...
		...
		hPage = [hPage, h];
 		...
 		for kkk=size(Perc,2):-1:1,
			...
			h     = text(0.37,y,sprintf(TabDat.fmt{2},Label(kkk).Nom),	'FontWeight','Bold',...
				'UserData',label,'ButtonDownFcn','get(gcbo,''UserData'')');
			hPage = [hPage, h];

			h     = text(0.60,y,sprintf(TabDat.fmt{3},Perc(kkk)),	'FontWeight','Bold',...
				'UserData',Perc(kkk),'ButtonDownFcn','get(gcbo,''UserData'')');
			hPage = [hPage, h];

			y = y - dy;
			[TabDat.dat{TabLin,1:3}] = deal(XYZmm(:,i),Label(kkk).Nom,Perc(kkk));
			TabLin = TabLin + 1;
		...
		if length(D) < Num
			...
			hPage = [hPage, h];
 			...
 			for kkk=size(Perc,2):-1:1,
				...
				h     = text(0.37,y,sprintf(TabDat.fmt{2},Label(kkk).Nom),...
					'UserData',label,...
					'ButtonDownFcn','get(gcbo,''UserData'')');
				hPage = [hPage, h];

				h     = text(0.60,y,sprintf(TabDat.fmt{3},Perc(kkk)),...
					'UserData',Perc(kkk),...
					'ButtonDownFcn','get(gcbo,''UserData'')');
				hPage = [hPage, h];
				y = y - dy;
				[TabDat.dat{TabLin,1:3}] = deal(XYZmm(:,d),Label(kkk).Nom,Perc(kkk));
			    TabLin = TabLin + 1;
